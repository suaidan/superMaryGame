...
// 精灵构造器。这里是构造函数，定义属性。
    SpriteCreate : function(name, painter, behaviors){
        function Sprite(name, painter, behaviors){
          ...
          this.height = 0;//高
          this.left = 0;//坐标中的x值
          this.cellNum = 0;//因为每个精灵的painter是个数组，这个表示的是当前使用的是哪个cell
          this.top = 0;//坐标中的y值
          this.v_X = 0;//x轴移动速度
          this.v_Y = 0;//Y轴移动速度
          this.aX = 0;//x轴方向的加速度
          ...
          this.lastTime = 0;//区别于引擎的lastTime，这里是具体的精灵对象使用的。
          this.energy = 0;//精灵对象的总能量，利用物理公式来来根据速度计算跳起来的高度。只和水平速度有关。
        };
        // 这里是构造函数的原型，定义方法。
        Sprite.prototype = {
            constructor : Sprite,
        ...
        SpriteSheetPainter.prototype.paint = function(sprite,context,i){//i表示是第几组
        ...
        return new Sprite(name,new SpriteSheetPainter(painter),behaviors)
    },
/***********************************************************************************************/
//游戏循环
    start : function() {
...
//输入处理,是个对象，里面提供方法
    input : {
        ...
        recordPress : function(event){
            ...
            var keyName = engineMario.input.translateKeyNum(keyNum);//!这里的调用方式不好，存在耦合
...
//初始化
    initialize : function(){
        ...
        var self = this;
        requestNextAnimationFrame(self.baseFunction.calculateFps);
        this.baseFunction.bindHandler("keydown",window,self.input.recordPress);
        this.baseFunction.bindHandler("keyup",window,self.input.recordUp);
    },
/***********************************************************************************************/
//擦除屏幕
